...
class AnalyticsPage(QWidget):
    ...
    def load_analytics_table(self) -> None:
        ...
        self.table.setColumnWidth(self.columns.index("choice"), 100)
        self.table.setColumnWidth(self.columns.index("analysis"), 110)
    ...
    def apply_direction_style(
        ...
        # --- FIZ_koof ---
        if column_name == "FIZ_koof":
            try:
                value = float(oi_row["FIZ_koof"])
                if value >= 2:
                    item.setBackground(Qt.GlobalColor.darkYellow)
                    item.setForeground(Qt.GlobalColor.white)
            except Exception:
        ...
        # --- YUR_koof ---
        if column_name == "YUR_koof":
            try:
                value = float(oi_row["YUR_koof"])
                if value >= 3:
                    item.setBackground(Qt.GlobalColor.darkYellow)
                    item.setForeground(Qt.GlobalColor.white)
            except Exception:
